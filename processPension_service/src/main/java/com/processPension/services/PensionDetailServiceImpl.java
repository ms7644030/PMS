...
public class PensionDetailServiceImpl implements ProcessPensionService{
	...
	public PensionDetail getPensionDetailByAadhar(Long aadhaar) {
		...
		//ResponseEntity<Object> response = restTemplate.getForEntity("http://PENSIONER-DETAIL-SERVICE/pensionerDetail/"+aadhaar,Object.class);
	...
	/*	if(response.getBody() instanceof PensionerDetail) {
		...
		ResponseEntity<String> response = restTemplate.getForEntity("http://PENSIONER-DETAIL-SERVICE/pensionerDetail/"+aadhaar,String.class);
		String value = (String) response.getBody();
